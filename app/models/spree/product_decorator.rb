Spree::Product.class_eval do
  ...
  # different value for it later (they can actively bypass by accessing each variant
  # directly and changing the values)
  ...
  def put_on_sale(value, calculator_type = "Spree::Calculator::DollarAmountSalePriceCalculator", all_variants = true, start_at = Time.now, end_at = nil, enabled = true)
    if all_variants && variants.present?
      variants.each do |v|
        v.put_on_sale(value, "Spree::Calculator::MasterDifferenceSalePriceCalculator", start_at, end_at, enabled)
      end
    end
